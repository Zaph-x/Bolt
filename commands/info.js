...
module.exports.run = async (message) => {
	...
	if (message.content.split(" ").length < 2) {
		return message.channel.send(new Discord.RichEmbed()
			...
			.setColor(red)
			.setTimestamp()
			.addField("I think we lost someone along the way!", "You didn't provide enough arguments to the command!")).then().catch(err => {
				console.log(err);
	...
	switch (args[0]) {
		...
		case "u":
			...
			embed.addField("Invite me", "[Click here!](https://discordapp.com/oauth2/authorize?client_id=473864695997136906&scope=bot&permissions=268443712)", true);
